...
Loading in the newest version of the data (8/4/21)


```{r}
hhs_all_complete_dg <- read_csv(here("new_portal_data", "hhs_all_complete.csv")) %>% 
  clean_names()
```
...
```{r}
global_filtered <- hhs_all_complete_dg %>%
  select(1:3,5, 7, 9:21,  )
...
global_filtered_numeric <- global_filtered %>%
  ...
  mutate(x45_leadership_position = 
    ...
    pattern = "No", replacement = "no")) %>%
  mutate(x45_leadership_position = ifelse(x45_leadership_position == "no",0,1)) %>%
  mutate(x10_mpa_important = 
           str_replace(x10_mpa_important,
    pattern = "Yes male", replacement = "yes")) %>% 
