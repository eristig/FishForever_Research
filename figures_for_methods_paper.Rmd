...
ggplot() +
  ...
  labs( x = "Agreement (Scaled across 5 Questions)",
      ...
      title = "Mapping the Fisher Typology",
      color = "Type") +
...
no_na_19 <- scaled_km_df %>% 
 ...
 mutate(x19_current_fish_catch = fct_relevel(x19_current_fish_catch, 
    "Declined a lot" = "Declined a lot",
          "Declined slightly" = "Declined slightly",
           "Stayed the same" = "Stayed the same",
          "Improved slightly" = "Improved slightly",
          "Improved heavily" = "Improved heavily"))
...
ggplot(reversed_x19_counts,
  ...
  labs( x = "Fisher Types",
      ...
      title = "Perceived Catch and  Fisher Type", 
      fill = "Perceived Change in Catch") +
  scale_fill_viridis(discrete = T) +
  theme_bw() +
   scale_y_continuous(labels = scales::percent) +
    theme(axis.text.x = element_text(angle = 45, hjust=1))
...
```
 ...
 "Are you confident that you will be able to procure enough food for you and your family for the next 12 months? **THis won't be used**
...
ggplot(swtiched_enforcement_counts,
  ...
  labs( x = "Fisher Type",
      y = "Percentage of Group",
      title = "Enforcement By Fisher Type", 
...
enforcement_sans_myself <- enforcement_counts %>% 
  filter(x49_enforcement_responsible != "Myself")


ggplot(enforcement_sans_myself,
...
x19_by_ma <- scaled_km_df %>% 
  select(x19_current_fish_catch, cluster_no, ma_name) %>% 
  drop_na() %>% 
  filter(x19_current_fish_catch != "Improved heavily")

ma_table <- x19_by_ma %>% 
  select(ma_name, x19_current_fish_catch) %>% 
  group_by(ma_name) %>% 
  count(x19_current_fish_catch) %>% 
  arrange(-n) %>% 
  pivot_wider(names_from = x19_current_fish_catch, values_from = n) %>% 
  drop_na() %>% 
  select(ma_name, 'Declined a lot', 'Declined slightly', 'Stayed the same', 'Improved slightly') %>% 
  adorn_totals("row") 
  
ma_table_props <- ma_table %>% 
    janitor::adorn_percentages(denominator = "row") %>% 
  adorn_pct_formatting(digits = 0) %>% 
  adorn_ns(position = "front")

kable(ma_table_props) %>% 
  kable_styling()
  ...
  #                     "Stayed the same" = "3", 
  #                     "Improved slightly" = "4")) %>% 
# mutate(x19_current_fish_catch = as.numeric(x19_current_fish_catch)) 




## table for average and SD

```

Now, we have a table of all 50 MAAs and their proportions broken down by perception of fish catch. 

**Eye Test Notes**
- There is very little consensus, particularly in the larger MAAs
- General Luna has a 66% response of "stayed the same." This is the largest % among the bigger MAs
- 45% of all respondents said "declined slightly," roughly double the next highest grouping 
- I'm curious of two things: Is there a "consensus" statistic at the level similar to a community? As in, if we KNOW that fish have gone down, and we asked a community, what would a "good" response be? That would liekly be very, very depedent on the type of question, huh? 
- Second, is Clsuter more predictive than geography, to see Perceived change in catch?


#### Figures of Big MAs

```{r}
ma_top_10 <- scaled_km_df %>% 
  select(x19_current_fish_catch, cluster_no, ma_name) %>% 
  mutate(x19_current_fish_catch = fct_relevel(x19_current_fish_catch, 
    "Declined a lot" = "Declined a lot",
          "Declined slightly" = "Declined slightly",
          ...
          "Improved slightly" = "Improved slightly",
          "Improved heavily" = "Improved heavily")) %>% 
  filter(ma_name %in% c('Santa Monica', 'Libertad', 'San Benito',
'Del Carmen', 'Culasi, Sebaste',
'General Luna', 'San Isidro, Pilar',
...
ggplot(ma_top_10, aes(x = ma_name, y = n, fill = x19_current_fish_catch)) +
 geom_col(position="dodge", alpha = 0.8) +
  theme_bw() +
  scale_fill_viridis(discrete = T) +
  labs(y= "Total Counts",
       x = "Managed Access Area", 
       fill = "Perceived Catch") +
   theme(axis.text.x = element_text(angle = 45, hjust=1))
```

Santa Monica, Libertad, San Benito,
Del Carmen, Culasi, Sebaste,
General Luna, San Isidro, Pilar,
RESEX Marinha Mocapajuba
...
**Site for Radar Chart syntax**
[https://www.r-graph-gallery.com/143-spider-chart-with-saveral-individuals.html]

```{r}
clusters_for_radar <- cluster_summary_df %>% 
  select(4:14) %>% 
  rename('Meeting.Attendance' = x44_mean, 
         'Leadership.Position' = x45_mean, 
         'Enforcement.Participation' = x48_mean, 
         'Behavior.Change' = x53_mean, 
         'Encourage.Neighbors' = x61g_mean,
         'M.A.Benefits' = x43_mean,
          'Represent.Interests' = x46_mean, 
         'Five.Years' = x52_mean, 
        'Effective.Regulations'  = x61a_mean,
        'Fairness' = x61f_mean, 
        'Trust.Local' = x30a_mean) %>% 
  data.frame()
  
rownames(clusters_for_radar) <- c("Committed", "Resistant", "Supportive", "Wavering")

max_min <- data.frame(
  'Meeting.Attendance' = c(1, -1),
  'Leadership.Position' = c (1, -1),
  'Enforcement.Participation' = c(1, -1), 
  'Behavior.Change' = c (1, -1),
  'Encourage.Neighbors' = c(5, 1),
  'M.A.Benefits' = c(1, 0),
  'Represent.Interests' = c(1, 0),
  'Five.Years' = c(2, -2),
  'Effective.Regulations'  = c(5, 1),
  'Fairness' = c(5, 1), 
  'Trust.Local' = c(5, 1))
...
radarchart(committed_radar, 
            pcol=rgb(0.9,0.6,0.1,0.9), pfcol=rgb(0.9,0.6,0.1,0.4) , plwd=2 , 
    #custom the grid
...
radarchart(resistant_radar, 
            pcol=rgb(0.2,0.6,0.9,0.9), pfcol=rgb(0.2,0.6,0.9,0.4) , plwd=2 , 
    #custom the grid
    cglcol="grey", cglty=1, axislabcol="gray", cglwd=1,
    #custom labels
    vlcex=0.6)
...
radarchart(supportive_radar, 
           pcol=rgb(0.1,0.9,0.9,0.9), pfcol=rgb(0.1,0.9,0.9,0.4) , plwd=2 , 
    #custom the grid
...
# Below is the Ambivalent radar

radarchart(wavering_radar, 
           pcol=rgb(0.7,0.2,0.5,0.9), pfcol=rgb(0.7,0.2,0.5,0.4) , plwd=2 , 
    #custom the grid
    cglcol="grey", cglty=1, axislabcol="grey", cglwd=1,
    #custom labels
    vlcex=0.6)

## Light blue, semi-transparent color:  pcol=rgb(0.2,0.5,0.5,0.9), pfcol=rgb(0.2,0.5,0.5,0.5)
```
