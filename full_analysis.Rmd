...
scaled_sum <- hhs_scaled %>% 
 rowwise() %>% 
  ...
  mutate(raw_agree_mean = mean(c(
    x43_ma_benefits + x46_represent_interests + x52_ma_benefit_5yrs + x61a_current_regulations + x61f_rights_distribution_fair))) %>% 
...
```{r}
 ggplot(data = scaled_sum,
   ...
   geom_point(color = "darkblue", size = 0.7, alpha = 0.7) +
  geom_point(data = neutral_point, aes(x = raw_agree_mean, y = raw_eng_mean),
             size = 8, color = "pink2") +
  labs( x = "Agreement (Scaled across 5 Questions)",
      y = "Engagement (Scaled across 5 Questions)",
      title = "Engagement and Agreement Index Mapping") +
...
agree_engage_unscaled <- scaled_sum %>% 
  select(24:33)
...
```{r}
corrplot(cor(agree_engage_unscaled), main = "Agreement + Engagement Question Correlations")
```
...
no_na_19 <- scaled_km_df %>% 
  ...
  mutate(x19_current_fish_catch = as_factor(x19_current_fish_catch)) %>% 
fct_relevel(x19_current_fish_catch, "Improved heavily", after = Inf)

  
 # as.factor(x19_current_fish_catch)
 # 
 # 
 #         "Declined a lot",
 #          "Decloned slightly",
