...
kmeans_basic_table
  ...
  km_table_means <- kmeans_basic_table %>% 
...
set.seed(1)
 ...
 scaled_agreement_engagement_km <- kmeans(agree_engage_only, 5)

## The 5 means the amount of clusters we're looking at


  scaled_agreement_engagement_km


  

 scaled_kmeans_basic_table <- data.frame(scaled_agreement_engagement_km$size, agreement_engagement_km$centers)

 scaled_kmeans_basic_table
 
 
 
 scaled_km_cl <- data.frame(scaled_sum, cluster_no = factor(scaled_agreement_engagement_km$cluster))
 
scaled_mean_cluster_df <- scaled_km_cl %>% 
        group_by(cluster_no) %>% 
...
* Agreement-only clusters, not in use.*
...
median_cluster_df <- agree_engage_cl %>% 
