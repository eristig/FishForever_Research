...
hhs_q7 <- read_csv(here::here("new_portal_data","hh_people_all.csv"))
...
Agreement includes questions 43, 46, 52, 61a, 61f, 30a
...
##### Comparing subquestions in x30 (Trusting different groups)
...
Lines 1172-1219 are filed in... `figures_for_methods_paper.Rmd`
```{r}
...
Lines 1281-1315, filed away in `mapping_indices_(part_2).Rmd
...
*Using x19 Counts to compare with Clusters*
```{r} 
x19_counts <- no_na_19 %>% 
  count(x19_current_fish_catch)
...
**Jitter Plot of Engagement and Clusters**
...
I'll clean up questions 17 and 18, and visualize them against compliance behaviors
...
Let's make a CSV of `scaled_km_df` so we can have a single .Rmd file for the Paper Figures and their asssociated tests. 

#### Chi-Squared Testing
Filed in... `figures_for_methods_paper.Rmd`

###### First Figure: Enforcement Responsibility 
...
Up Next, Fishing Effort in Offseason 
...
Radar plots for Clusters

We need a dataframe with the compliance questions and Cluster Numbers
```{r}

full_spider_df <- scaled_km_df %>% 
  select(x44_meeting_attendance, x45_leadership_position, x48_enforcement_participation, 
        x53_encourage_regulations, x61g_fishing_change_behavior, 
        x43_ma_benefits, x52_ma_benefit_5yrs, x61a_current_regulations,
        x46_represent_interests, x61f_rights_distribution_fair,
             x30_trust_local_decision)

radarchart(full_spider_df)
```