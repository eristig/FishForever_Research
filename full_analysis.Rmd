...
- `hhs` `441, `45`, and `48`, questions that needed to be joined with the larger df
...
```
*Figure 1: A plot with the **neutral* fisher reported within the of our Z-score-based indices. We see that Ms. Neutral is 1 SD below the mean in agreement. This location means that the vast majority of fishers at least somewhat agree with rules in place. I hypothesize that (with Rare's perspective) that changing *engagement* of fishers in the opportunity; improving agreement is important, but we see more measurable room for change on the engagement axis.* 
...
```
*Figure 2: Density plot of the 4500 hundred responses. This allows us to see where points overlap, which was challenging in the first figure. We see a pocket of 100 responses at (0,1) which may be the leaders, and several pockets of over 100 at (0,-0.5) and at the -2.5 horizon of engagement* 
...
```{r}
 ...
 geom_point(size = 0.7, alpha = 0.7) +
  theme_bw()
```
*Figure 3: Mapping all A/E responses by the raw sum of their answers. This gives us a huge range of scores*
...
Within Agreement, the average response is positive, msot folks respond 1 sd above Neutral. With Engagement, we see a a mean of 0.6, which is right at "neutral" and a more normal distribtuion from there. 
...
- but there is only ONE! observation from brazil that answered questions 62 and 64 
...
fit <- cfa(model1, data = agree_engage_only)
...
set.seed(2)
 ...
 scaled_km <- kmeans(agree_engage_only_scaled, 3)
...
kmeans_kable
```
C1:  Mean at (0.22, -0.35) We see values close to 0 for both Agreement and Engagement. None of these responses attend meetings, a decent amount are willing to change behavior. Positive values across Agreement. 
C2: Mean at (0.24, 0.0) Cluster 1 and 2 are very close, but these Cluster 2 are the average fishers who DO attend local meetings. 

In fact, clusters 2 and 5 nearly all attend meetings while some folks in Cluster 3 attend meetings (range of -1.5-1)


Cluster 5 are leaders, Cluster 2 have a barely positive value for leadership.
Enforcement is negative among all clusters EXCEPT 5 (Leader/engaged Cluster)
Let me get the exact amounts. 
...
```
*Figure 5: Cluster 1 has 2 density points right next to one another. About 200 of the 1100 reside between -0.75 and 0.25 Agreement, and -0.5 Engagement. They're below average on engagement and right near the mean (0,0) point for agreement.*
...
Dyl hypothesis: National government would show us less engaged fishers, while fishers will be more engaged when Fisheries Management Boards take responsibility for enforcement. 

Curious question: What is the breakdown of those who say "Myself"? What is the N, where are they from, and how do they find themselves in Cluster?
...
ggplot(three_filter_enforcement,
       aes(x = agree_mean, y = eng_mean, color = x49_enforcement_responsible, shape = x49_enforcement_responsible)) +
  geom_point(size = 0.7) +
  labs( x = "Agreement (Scaled across 5 Questions)",
      y = "Engagement (Scaled across 5 Questions)",
