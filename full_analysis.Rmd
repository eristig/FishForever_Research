...
x61i_help_neighbors,
x66_reaction_fishing_reserve, x66_response_fishing_reserve, x62_reserve_compliance, x63_fishing_in_reserve, x64_wrong_fishing_reserve)) %>% 
...
collapse_rows(columns = 3, valign = "middle") %>% 
  column_spec(1:3, border_right = TRUE, color = "black")
...
labs( x = "Agreement (Scaled across 6 Questions)",
      y = "Engagement (Scaled across 5 Questions)",
      title = "Mapping Attitudes of Admitted rule-breakers", 
      color = "Country") +
  scale_x_continuous(limits = c(-3, 2),
                     breaks = c(-2, -1, 0, 1, 2)) +
   scale_y_continuous(limits = c(-1.5, 2),
...
# NbClust(agree_engage_only[1:10], min.nc = 2, max.nc = 8, method = "kmeans")


set.seed(1)

 agreement_engagement_km <- kmeans(agree_engage_only, 5)
...
## The 5 means the amount of clusters we're looking at
 ...
 kmeans_basic_table <- data.frame(agreement_engagement_km$size, agreement_engagement_km$centers)

 kmeans_basic_table

  kmeans_kable <- head(kmeans_basic_table) %>% 
...
```
There are *either* 2 or 5! 
574, 853, 731, 1369, 922
...
```{r}
agree_engage_cl <- data.frame(scaled_sum, cluster_no = factor(agreement_engagement_km$cluster))


#### Below is a summary df that shows all the means

mean_cluster_df <- agree_engage_cl %>% 
        group_by(cluster_no) %>% 
        summarise(eng_mean = mean(eng_mean),
                  agree_mean  = mean(agree_mean)) 

cluster_counts <- agree_engage_cl %>% 
  count(cluster_no)



ggplot() +
  geom_point(data = agree_engage_cl,
       aes(x = agree_mean, y = eng_mean,
           color = cluster_no),
           size = 0.7, alpha = 0.7) +
...
Center = (-0.1748106, 0.002805289) (Engagement, Agreement)
N = 936

*Cluster 4* 
Its center is quite close to Cluster 3, but we can see clear differences when we look at the spread. These folks do NOT engage. While there as unengaged as the Resistant fishers, but most actually agree quite a lot. This includes some with A = 1, the max. 
Center = (-0.4098127,  0.052635489) (Engagement, Agreement)
...
Center = (-0.4569608,  -0.837729407) (Engagement, Agreement)
...
**Quickly, I'll look at the primary graph with 2 clusters.**

```{r}
agreement_engagement_km_2 <- kmeans(agree_engage_only, 2)

## The 5 means the amount of clusters we're looking at


  agreement_engagement_km_2
  
  agree_engage_cl_2 <- data.frame(scaled_sum, cluster_no = factor(agreement_engagement_km_2$cluster))
  
  
  ggplot() +
  geom_point(data = agree_engage_cl_2,
       aes(x = agree_mean, y = eng_mean,
           color = cluster_no),
           size = 0.7, alpha = 0.7) +
  labs( x = "Agreement (Scaled across 6 Questions)",
      y = "Engagement (Scaled across 5 Questions)",
      title = "2 Cluster TEST MAP", 
      color = "Cluster") +
  scale_x_continuous(limits = c(-3, 2),
                     breaks = c(-2, -1, 0, 1, 2)) +
   scale_y_continuous(limits = c(-1.5, 2),
...
```
Ok, this doesn't really tell us much! 


```{r}
ggplot(data = agree_engage_cl,
...
gender_mean <- agree_engage_cl %>% 
  group_by(x6_gender) %>% 
  summarise(eng_mean = mean(eng_mean),
                  agree_mean  = mean(agree_mean))

ggplot() +
  geom_jitter(data = agree_engage_cl, 
...
```{r}
comparing_clusters <- agree_engage_cl %>% 
...
# cluster_number_est_3 <- NbClust(agree_scaled_sum[1:6], min.nc = 2, max.nc = 8, method = "kmeans")

# cluster_number_est_3
...
Graphing 3 Clusters
```{r}
ggplot(data = agree_cl,
       aes(x = country, y = agree_mean, color = cluster_no)) +
  geom_jitter()
```
...
Notes:
We see that this is determined by low engagement, but decent to high agreement. The cluster ranges from -1 to 1 on agreement. These folks are OK with rules for the most part. However, they don't take part in the management process
...
```
Notes: 
Just as with the full graph, this has the most spread, centered very close to (0,0.)
on Engagement we se -1 up to 0.5 and -0.5 to 1 with agreement. 
...
Notes: 0 engagement is the floor. These folks are engaged. We see a maximum engagement score of 1. I'll investigate what question responses determine this. 
BRA: 26
FSM: 3
FSM: 
HND: 27
...
With a mean at (-0.5, -8) this is a disagreeing, unengaged group, including some crazy disagreeing outliers, from Honduras. There are a chunk of observations above 0 in engagement, but only 1 on the agreement axis. 
...
# Making a "binding" df where the cluster of each observation is attached to it 
...
```{r}
corrplot(cor(agree_engage_only), main = "Agreement + Engagement Question Correlations")
```
