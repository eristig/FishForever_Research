...
- *For Questions 44 and 45* 
 ...
 - I've since removed this (20 observation decrease.) I think it's better to be uniforrm with all these questions, since there really was a small number of such responses. 
...
hhs_scaled_character <- hhs_scaled %>% 
  as.character(c(24:28))
  

hhs_scaled_sum <- hhs_scaled %>%
  select(24:28) %>% 
...
df_eng_stats <- data.frame(hhs_scaled) 
cols <- c(24:28)  
stargazer(
    df[, cols], type = "text", 
...
df_agree_stats <- data.frame(hhs_scaled) 
cols <- c(29, 30, 32:38)  
stargazer(
    df[, cols], type = "text", 
    summary.stat = c("min", "p25", "mean", "p75", "max", "median", "sd"))

## WE HAVE SOME STATS doube-checking to do. 
...
`hhs_scaled` is the df that will be used for all analysis. 
...
### First I'll jsut select the engagement variables 

engagement_only <- scaled_sum %>% 
  select(24:28)

engagement_km <- kmeans(engagement_only, 4) ## The 4 means the amount of clsuters we're looking at
...
ggplot(engagement_cl) +
  geom_point(aes(x = level2_name, y = eng_mean, color = cluster_no))

```
OK, honestly, I do NOT know how to cluster these. 
...
Starting with the euclidean distances 

```{r}

##  Scaling the 5 engagement questions

engagement_only_scaled <- engagement_only %>% 
  scale()

euc_distance_eng <- dist(engagement_only_scaled, method = "euclidean")

euc_distance_eng

hc_complete_engagement <- hclust(euc_distance_eng, method = "complete" )


## Plotting this heirarchical cluster


plot(hc_complete_engagement, cex = 0.6, hang = -1)
...
we're returning to this Tuesday. 
