...
* Agreement-onyl clusters, not in use.*
...
```
Counts: 
BRA 25, HND 29, IDN 33, MOZ 322, PHL 422


Notes:
Committed fishers, easily the highest engaging group. Like C1, has a good chunk of high-agreeing fishers also. Those top engagement scores seem to come from nearly all countries (less from Honduras.)
...
on Engagement we see -1 up to 0.5 and -0.5 to 1 with agreement. 
...
enforcement_df <- agree_engage_cl %>% 
...
ggplot(enforcement_counts,
  ...
  labs( x = "Enforncement Responsibility",
      y = "Cluster",
...
```{r}
boat_ownership <- agree_engage_cl %>% 
  select(eng_mean, agree_mean, x21_boat_owner_status, cluster_no, x23_job_secure, country)
...
Mozambique has the highest percentage of fishers who fish without boats, even more than Brazil (where I know many fishers collect crabs and other invertibrates)
...
first_correlate_df <- agree_engage_cl %>% 
  ...
  mutate(cluster_no = as.numeric(cluster_no)) %>% 
...
corr_1 <-  corrplot(cor(first_correlate_df, use="pairwise.complete.obs"), main = "Lots of variables")
